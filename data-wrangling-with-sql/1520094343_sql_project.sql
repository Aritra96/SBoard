...
SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname, ';', f.name) AS name_and_courtname
FROM country_club.Members AS m, country_club.Bookings AS b, country_club.Facilities AS f
WHERE m.memid = b.memid
AND f.name LIKE '%Tennis C%'
ORDER BY 1; 
...
SELECT CONCAT(m.firstname, ' ', m.surname, '; ', f.name ) AS people_and_facility_names, 
...
FROM (SELECT * 
      ...
      WHERE starttime BETWEEN '2012-09-14' AND '2012-09-15') AS b
...
SELECT f.name AS facilities, SUM(
